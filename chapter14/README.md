...
- 我引入composer包管理创建并维护了新的开发框架；
- 框架基于Slim v3.7，是小组之前依赖的，因考虑过渡成本暂时没改变。

## 场景问题

近期发现自己在做一些项目中经常遇到的业务接口有一些共同特点：

- 一个接口中部分逻辑（比如下方栗子中的1/3/5，后续简称主逻辑）是需要保障处理成功并将结果反馈给调用方的；
- 而另一部分逻辑（比如下方栗子中的2/4，后续简称支逻辑）则是可以容忍失败或暂时失败，甚至调用方并不是很关心结果或者说感知不明显；
- 大部分支逻辑都不算非常复杂，往往是简单判断加上打个文件日志、写条MySQL日志记录、发个HTTP请求等类；
- 主支逻辑在代码编写顺序上往往是交叉混淆而非泾渭分明的；
...
先闭上眼睛什么都不考虑一把梭顺序编写代码实现，执行流程应该是如下这样的（事实上一开始我就是这么一把梭实现的）：
...
面对这种场景FPM运行模式的PHP一般会选择什么方案呢？也许你也不会陌生——进程级别的异步任务方案！Laravel/Lumen也是采用这个方案的，执行流程如下：
...
当然还有很多其他方案模型，不过也都比较复杂，而且都不利于保持CGI+同步阻塞这套模型给团队同学的门槛好处和对服务的稳定安全，所以也不是本文的主角～

> 特别是今年团队的主基调是质量、效率

# 思路

如果想保留CGI+同步阻塞这套运行模型，看起来上述提到的运行模型是比较好的选择，但不难发现它存在着另一些问题：
...
> 当然这里不是说这套方案就没有可用之处，只是场景匹配度的问题。

## 分析转换

那么有没有什么（使用成本和运行效率上）轻量级又保留现有优势的方案呢？

---

思考分析：

- 既然仍然是采用同步阻塞方案，也就是说不去做串行改并行的优化，仍旧是原来的执行时间；
- 既然想要轻量级，那就把队列服务、消费进程都干掉，仍旧由当前处理该请求的FPM处理进程来执行全部逻辑；
...
2. 能不能顺序编写代码逻辑，但执行时将指定部分的多块代码逻辑块延迟到某个指定逻辑之后？（这是因为我需要考虑封装成方便大家使用的框架能力）

## 问题1解法：fastcgi_finish_request

想到了这个函数，PHP在FPM下这个方法是可用的，且其功能就是上述问题1！！！

---

另外印象里Laravel/Lumen有个终结者中间件，好像也是实现了类似功能（先返回响应给调用方，再继续执行终结者中间件逻辑），于是打算去翻翻源码回忆验证一下其实现原理：

> - 对这个有印象是因为当年有同事使用Laravel时遇到通过框架提供的session修改和保存方法但是并没有生效的问题
> - 我协助排查的时候大概看到过框架对于session的操作都是在进程级别的，只有在终结者中间件中才会将内存中的session完整覆盖到存储驱动中
> - 问题是同事居然在一通逻辑处理后没有采用框架提供的response方法返回响应，直接echo然后exit，以至于没法走到框架后续的返回响应和执行终结者中间件。。。
> - 所以记忆犹新～～

---

通过Laravel源码不难看出，确实也是通过fastcgi_finish_request来实现此功能的，当然它对其他运行模式也做了兼容，但是我暂时用不到。
...
前文说到我要考虑框架封装方便组内同学使用的，所以不能裸写而要考虑大家使用时的便捷性，如果可以同步编写代码逻辑块，但在执行顺序上将标记的代码逻辑块延迟到请求响应返回后继续执行就很理想了～
...
理论上也可以做成像go的defer任意嵌套，但既然是延迟到请求返回响应后，那么嵌套没意义，反而会增加代码编写和阅读的复杂性，所以干脆不支持。
